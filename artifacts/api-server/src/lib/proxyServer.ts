...
export function handleConnect(req: IncomingMessage, clientSocket: net.Socket, head: Buffer): void {
  const authHeader = req.headers["proxy-authorization"] as string | undefined;
  if (!validateProxyAuth(authHeader)) {
    logger.warn({ url: req.url }, "CONNECT: auth failed");
  ...
  getRandomWorkingProxy()
    .then((upstream) => {
      if (!upstream) {
        logger.warn({ targetUrl }, "CONNECT: no working proxy in pool");
        recordRequest({
      ...
      upstreamSocket.on("connect", () => {
        ...
        const onUpstreamData = (chunk: Buffer) => {
          ...
          clientSocket.on("error", (err) => {
            logger.debug({ err }, "CONNECT: client socket error");
            upstreamSocket.destroy();
          });
          upstreamSocket.on("error", (err) => {
            logger.debug({ err }, "CONNECT: upstream socket error");
      ...
      upstreamSocket.on("error", (err) => {
        ...
        if (!settled) {
          settle();
          logger.warn({ err, upstream }, "CONNECT: upstream connection error");
          recordRequest({
            targetUrl: `https://${targetUrl}`,
            method: "CONNECT",
            latency: Date.now() - connectStart,
      ...
      clientSocket.on("error", (err) => {
        logger.debug({ err }, "CONNECT: client socket error during setup");
        upstreamSocket.destroy();
      });
    })
    .catch((err) => {
      logger.error({ err }, "CONNECT: error getting upstream proxy");
      rejectSocket(clientSocket, 503, "Service Unavailable");
    });
}

export function handleHttpProxy(req: IncomingMessage, res: ServerResponse): void {
  const proxyStart = Date.now();
  const authHeader = req.headers["proxy-authorization"] as string | undefined;
  if (!validateProxyAuth(authHeader)) {
    logger.warn({ url: req.url }, "HTTP proxy: auth failed");
    res.writeHead(407, {
  ...
  getRandomWorkingProxy()
    .then((upstream) => {
      if (!upstream) {
        logger.warn({ url: req.url }, "HTTP proxy: no working proxy in pool");
        recordRequest({
      ...
      proxyReq.on("error", (err) => {
        logger.warn({ err, upstream, url: req.url }, "HTTP proxy: upstream request error");
        recordRequest({
          targetUrl: req.url ?? "unknown",
          method: req.method ?? "GET",
          latency: Date.now() - proxyStart,
    ...
    .catch((err) => {
      logger.error({ err }, "HTTP proxy: error getting upstream proxy");
      if (!res.headersSent) {
        res.writeHead(503, { "Content-Type": "text/plain" });
